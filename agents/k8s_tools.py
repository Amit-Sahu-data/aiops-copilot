...
def get_pod_resource_limits(namespace: str = "default") -> str:
    ...
    try:
        ...
        return f"Failed to fetch resource limits after retries: {e.reason} (status {e.status})"
...
def get_deployment_info(namespace: str = "default") -> str:
    ...
    try:
        ...
        return f"Failed to fetch deployment info after retries: {e.reason} (status {e.status})"
...
def _patch_deployment_with_retry(deployment_name, namespace, body):
    return apps_v1.patch_namespaced_deployment(name=deployment_name, namespace=namespace, body=body)
